...
var _AuthService = class _AuthService {
  ...
  login(email, password) {
    return this.http.get(`${this.baseUrl}/users/1 `).pipe(tap((user) => this.user = user), tap((user) => localStorage.setItem("token", "aASDgjhasda.asdasd.aadsf123k")));
  }
  checkAuthentication() {
    if (!localStorage.getItem("token"))
      return of(false);
    const token = localStorage.getItem("token");
    return this.http.get(`${this.baseUrl}/users/1 `).pipe(tap((user) => this.user = user), map((user) => !!user), catchError((err) => of(false)));
  }
  logout() {
    this.user = void 0;
    localStorage.clear();
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-INFC27TE.js.map
